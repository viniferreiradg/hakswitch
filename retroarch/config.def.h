...
/* HAKSWITCH TEST: RetroArch's own historical default swaps OK/Cancel
 * (B confirms, A cancels) - our on-screen HUD hints say "(A) Selecionar
 * / (B) Voltar", so force the modern A-confirms convention to match. */
...
/* HAKSWITCH TEST: on by default so the carousel has sound feedback
 * (scroll/select/back) out of the box - stock RetroArch ships these
 * off. Notice/BGM left off, not part of the requested feedback set. */
