...
/* HAKSWITCH TEST: hakswitch_current_path/hakswitch_root_path live in
 * menu_displaylist.c, which owns the folder scan that builds our
 * carousel entries; these OK handlers are what runs when the user
 * actually presses OK on one of those entries. */
 ...
 * items) - entering a console from the root jumps straight into its
 * "roms" subfolder instead of showing those 3 folders as choices. */
static int action_ok_hakswitch_enter_dir(const char *path,
      const char *label, unsigned type, size_t idx, size_t entry_idx)
{
   char tmp[1024];
   struct menu_state *menu_st = menu_state_get_ptr();
   bool at_root = string_is_equal(hakswitch_current_path, hakswitch_root_path);

   fill_pathname_join_special(tmp, hakswitch_current_path, path, sizeof(tmp));
   if (at_root)
      fill_pathname_join_special(tmp, tmp, "roms", sizeof(tmp));
   strlcpy(hakswitch_current_path, tmp, sizeof(hakswitch_current_path));

   /* A remembered last-played game only makes sense in the folder it
    * was launched from - moving to a different folder invalidates it. */
   hakswitch_last_played_path[0] = '\0';

   /* Custom handler bypasses RetroArch's own default OK callbacks
    * (where this normally happens), so it needs its own sound cue. */
   audio_driver_mixer_play_menu_sound(AUDIO_MIXER_SYSTEM_SLOT_OK);
...
/* Holds a game launch that's been confirmed (core found, "launch"
 * sound started) but not yet actually handed to
 * task_push_load_content_with_new_core_from_menu - see
 * hakswitch_check_pending_launch just below for why. */
static bool     hakswitch_pending_launch_active             = false;
static char     hakswitch_pending_launch_core_path[1024]     = "";
static char     hakswitch_pending_launch_content_path[1024]  = "";
static retro_time_t hakswitch_pending_launch_time            = 0;

/* task_push_load_content_with_new_core_from_menu reinitializes the
 * audio driver for the new core almost immediately, which cuts off
 * the "launch" sound (HAKSWITCH_LAUNCH_SFX_SLOT) before a single
 * buffer of it reaches the speakers if fired in the very same call -
 * confirmed by logging the mixer slot's state around the call: it
 * does flip STOPPED -> PLAYING, so the voice genuinely starts, it
 * just never gets heard. Waiting a beat lets it actually play before
 * the load tears the driver down. Checked once per frame from
 * ozone_frame() (menu/drivers/ozone.c). */
#if defined(RARCH_CONSOLE)
/* Real-hardware testing on Switch surfaced two problems with the
 * deferred approach below:
 * 1) armGetSystemTick() (what cpu_features_get_time_usec() reads on
 *    libnx) is per-CPU-core and isn't guaranteed consistent if the OS
 *    migrates this thread to a different core mid-wait - elapsed time
 *    could get stuck below the threshold indefinitely.
 * 2) More seriously: even with the delay at 0, deferring the actual
 *    task_push_load_content_with_new_core_from_menu() call to fire
 *    from hakswitch_check_pending_launch() - called every frame from
 *    ozone_frame() - moves it from the OK-handler's input-processing
 *    context into the middle of the video frame-draw callback. On
 *    Switch that call triggers a real process handoff (envSetNextLoad,
 *    see frontend/drivers/platform_switch.c); firing that mid-frame,
 *    with GPU/graphics state mid-draw, reproduced the exact
 *    freeze-then-crash reported on hardware. Bypassed entirely on
 *    console: action_ok_hakswitch_open_file() below calls the load
 *    directly, the same call context proven stable before the
 *    pending-launch mechanism existed. */
#define HAKSWITCH_LAUNCH_SFX_CONSOLE_DIRECT 1
#else
/* launch.wav runs ~2.05s (44.1kHz stereo, 90517 frames) - give it the
 * full length plus a small buffer before handing off to content-load,
 * or the audio driver reinit cuts it off mid-playback. */
#define HAKSWITCH_LAUNCH_SFX_DELAY_USEC 2150000
#endif

void hakswitch_check_pending_launch(void)
{
#if !defined(HAKSWITCH_LAUNCH_SFX_CONSOLE_DIRECT)
   content_ctx_info_t content_info;

   if (!hakswitch_pending_launch_active)
      return;
   if ((cpu_features_get_time_usec() - hakswitch_pending_launch_time)
         < HAKSWITCH_LAUNCH_SFX_DELAY_USEC)
      return;

   hakswitch_pending_launch_active = false;
   ...
   task_push_load_content_with_new_core_from_menu(
         hakswitch_pending_launch_core_path,
         hakswitch_pending_launch_content_path,
         &content_info, CORE_TYPE_PLAIN, NULL, NULL);
#endif
   /* On console the load is fired directly from
    * action_ok_hakswitch_open_file() instead - see
    * HAKSWITCH_LAUNCH_SFX_CONSOLE_DIRECT above. hakswitch_pending_launch_active
    * is never set to true on console, so hakswitch_is_launch_pending()
    * below correctly reports "not pending" and the loading spinner
    * simply never gets a window to draw in on that platform. */
}

/* ozone.c reads this to know whether to draw the "loading" spinner
 * under the selected card - true for exactly the same dead window
 * hakswitch_check_pending_launch above is waiting out. */
bool hakswitch_is_launch_pending(void)
{
   return hakswitch_pending_launch_active;
}

/* Detects the right core by file extension and loads core+content in
 * one shot - same helper RetroArch itself uses for "start this core
 * with this content" from other menu actions (see
 * task_push_load_content_with_new_core_from_menu in task_content.c).
 * Loading this way (not through a playlist) makes RetroArch's own
 * "Close Content" flow return to our Main Menu tab afterwards, since
 * that's runloop.c's default flush target. */
static int action_ok_hakswitch_open_file(const char *path,
      const char *label, unsigned type, size_t idx, size_t entry_idx)
{
   char fullpath[1024];
   ...
   fill_pathname_join_special(fullpath, hakswitch_current_path, path, sizeof(fullpath));

   core_info_get_list(&core_list);
   if (core_list)
      core_info_list_get_supported_cores(core_list, fullpath, &info, &supported);

   if (supported == 1 && info)
   {
      /* Distinct from the generic "select" sound (OK on a folder) -
       * this one specifically means "a game is starting". Preloaded
       * once by hakswitch_init_paths() (menu_displaylist.c) into
       * HAKSWITCH_LAUNCH_SFX_SLOT - playing that already-loaded slot
       * synchronously here (confirmed via logging: the voice really
       * does start, state flips STOPPED -> PLAYING) is not enough on
       * its own though - task_push_load_content_with_new_core_from_menu
       * below reinitializes the audio driver for the new core so fast
       * that not a single buffer of it reaches the speakers. Actually
       * starting the load is deferred a moment (see
       * hakswitch_check_pending_launch below) so the sting is
       * audible first. */
      audio_driver_mixer_play_stream(HAKSWITCH_LAUNCH_SFX_SLOT);

      /* Visual "confirm" cue to go with it - selected cover bumps up
       * ~3% and back (ozone.c owns the animation, since it owns the
       * carousel draw). */
      hakswitch_trigger_select_pulse();

      /* Remembered so DISPLAYLIST_MAIN_MENU can re-select this exact
       * game once we're back from "Close Content". */
      strlcpy(hakswitch_last_played_path, fullpath, sizeof(hakswitch_last_played_path));

      /* Also persisted to disk: on Switch, "Close Content" comes back
       * up inside a different process (the core's own .nro - see
       * frontend/drivers/platform_switch.c), so the in-memory globals
       * above won't survive the trip. hakswitch_load_last_played_state()
       * in menu_displaylist.c reads this back exactly once at process
       * start. */
      {
         ...
         state_f = fopen(state_path, "w");
         if (state_f)
         {
            fprintf(state_f, "%s\n%s\n", hakswitch_current_path, fullpath);
            fclose(state_f);
         }
      }

#if defined(HAKSWITCH_LAUNCH_SFX_CONSOLE_DIRECT)
      /* Called directly, from right here in the OK-handler - see the
       * HAKSWITCH_LAUNCH_SFX_CONSOLE_DIRECT comment above for why this
       * differs from the deferred PC path below. */
      {
         content_ctx_info_t content_info;
         ...
         task_push_load_content_with_new_core_from_menu(
               info->path, fullpath, &content_info, CORE_TYPE_PLAIN, NULL, NULL);
      }
#else
      /* Deferred rather than called directly - see
       * hakswitch_check_pending_launch below for why. */
      strlcpy(hakswitch_pending_launch_core_path, info->path,
            sizeof(hakswitch_pending_launch_core_path));
      strlcpy(hakswitch_pending_launch_content_path, fullpath,
            sizeof(hakswitch_pending_launch_content_path));
      hakswitch_pending_launch_time   = cpu_features_get_time_usec();
      hakswitch_pending_launch_active = true;
#endif
   }
   else
   {
      ...
      size_t _len = (size_t)snprintf(msg, sizeof(msg),
            "HakSwitch: nenhum core encontrado para %s (%u compativeis)",
            path, (unsigned)supported);
      ...
      const char *menu_label, size_t menu_lbl_len)
{
   ...
   /* HAKSWITCH TEST: intercept our own carousel folder/file entries
    * before the stock label/type dispatch below, which doesn't know
    * about them and was sending directory clicks into RetroArch's
    * favorites/content-list flow (hence the earlier "Directory Not
    * Found" error). */
   if (string_is_equal(label, "hakswitch_dir"))
   {
      BIND_ACTION_OK(cbs, action_ok_hakswitch_enter_dir);
      return 0;
   }
