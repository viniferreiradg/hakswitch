...
/* HAKSWITCH TEST: hakswitch_root_path/hakswitch_current_path live in
 * menu_displaylist.c (the folder scan that builds our carousel
 * entries) and menu_cbs_ok.c (OK handlers for entering/leaving a
 * folder) - we need them here too, to know whether the carousel is
 * currently showing the console list (root) or a console's game list,
 * and to find each console's logo/background image files. */
extern char hakswitch_base_path[1024];
extern char hakswitch_root_path[1024];
extern char hakswitch_current_path[1024];
extern char hakswitch_last_played_path[1024];
/* menu_cbs_ok.c - defers actually loading content a moment after the
 * "launch" sound starts, so it's audible before the new core's load
 * tears the audio driver down. Pumped once per frame here. */
extern void hakswitch_check_pending_launch(void);
/* menu_displaylist.c - re-preloads the "launch" sound if "Close
 * Content" wiped it out from under us (CMD_EVENT_AUDIO_REINIT tears
 * down/rebuilds the whole audio driver). Cheap no-op otherwise -
 * pumped once per frame here too. */
extern void hakswitch_ensure_launch_sfx_loaded(void);
/* True during the same dead window hakswitch_check_pending_launch is
 * waiting out - drives the loading spinner below. */
extern bool hakswitch_is_launch_pending(void);
/* menu_displaylist.c - keeps a random track from hakswitch_base_path/
 * music/ playing at all times, self-healing like the launch sfx
 ...
 * and the delay before content-load actually kicks in (both driven
 * from menu_cbs_ok.c, which calls hakswitch_trigger_select_pulse()
 * below), similar to the cover bump seen on console UIs when picking
 * a game. logo_scale multiplies this in below, in the carousel's
 * per-slot draw loop. */
static float hakswitch_select_pulse_scale = 1.0f;

static void hakswitch_select_pulse_return(void *userdata)
{
   struct gfx_animation_ctx_entry entry;
   uintptr_t tag = (uintptr_t)&hakswitch_select_pulse_scale;

   entry.duration     = 160.0f;
   entry.target_value = 1.0f;
   entry.subject      = &hakswitch_select_pulse_scale;
...
void hakswitch_trigger_select_pulse(void)
{
   struct gfx_animation_ctx_entry entry;
   uintptr_t tag = (uintptr_t)&hakswitch_select_pulse_scale;

   gfx_animation_kill_by_tag(&tag);
   hakswitch_select_pulse_scale = 1.0f;

   entry.duration     = 110.0f;
   entry.target_value = 1.03f;
   entry.subject      = &hakswitch_select_pulse_scale;
   entry.easing_enum  = EASING_OUT_QUAD;
   entry.tag          = tag;
   entry.cb           = hakswitch_select_pulse_return;
   entry.userdata     = NULL;

   gfx_animation_push(&entry);
}

/* HAKSWITCH DIAGNOSTIC BUILD: define this to skip every hakswitch image
 * load below (console logos/backgrounds, game covers/screenshots, the
 * no-cover placeholder, HUD button icons) - texture handles are left at
 * 0, and every draw call already null-checks its texture before drawing
 * (grep "->tex)" in this file), so the rest of the carousel/menu runs
 * exactly as normal, just with blank art. Built once to isolate whether
 * synchronous PNG decode is what's causing the freezes reported on
 * Switch: if a build with this defined doesn't freeze, image loading is
 * confirmed as the cause. Not meant to ship - comment back out after. */
/* #define HAKSWITCH_DIAG_NO_IMAGES 1 */

/* HAKSWITCH TEST: each console folder (consoles/<Name>/) has "logo"
 * and "background" subfolders with exactly one image file each. Looked
 * up and cached lazily per console name the first time it's drawn,
 * instead of hardcoding which systems have art like the first version
 * of this test did. */
typedef struct
{
   char name[64];
   uintptr_t logo_tex;
   unsigned  logo_w, logo_h;
   uintptr_t bg_tex;
   unsigned  bg_w, bg_h;
   bool tried;
} hakswitch_console_asset_t;
 ...
 * decodes off the main thread instead of blocking the frame like
 * gfx_display_reset_textures_list does. Needed here too - confirmed by
 * the user hitting the same freeze returning to the console list right
...
{
   int i;
   hakswitch_console_asset_t *slot = NULL;

   for (i = 0; i < hakswitch_console_asset_count; i++)
   {
      if (string_is_equal(hakswitch_console_assets[i].name, console_name))
      {
         slot = &hakswitch_console_assets[i];
         break;
      }
   }

   if (!slot && hakswitch_console_asset_count < 8)
   {
      slot = &hakswitch_console_assets[hakswitch_console_asset_count++];
      strlcpy(slot->name, console_name, sizeof(slot->name));
      slot->logo_tex = slot->bg_tex = 0;
      slot->logo_w = slot->logo_h = slot->bg_w = slot->bg_h = 0;
      slot->tried    = false;
   }
   ...
   if (slot && !slot->tried)
   {
      ...
      slot->tried = true;
      fill_pathname_join_special(console_dir, hakswitch_root_path, console_name, sizeof(console_dir));

      fill_pathname_join_special(asset_dir, console_dir, "logo", sizeof(asset_dir));
      if ((files = dir_list_new(asset_dir, NULL, true, false, false, false)))
      {
         if (files->size > 0)
         {
      ...
      fill_pathname_join_special(asset_dir, console_dir, "background", sizeof(asset_dir));
      if ((files = dir_list_new(asset_dir, NULL, true, false, false, false)))
      {
         if (files->size > 0)
         {
 ...
 * background art. */
static void hakswitch_draw_texture_cover(gfx_display_t *p_disp, void *userdata,
      unsigned video_width, unsigned video_height,
      uintptr_t tex, unsigned tex_w, unsigned tex_h, float *color)
{
   float scale = (tex_w && tex_h)
         ? MAX((float)video_width / tex_w, (float)video_height / tex_h)
         : 1.0f;
   int draw_w  = (int)(tex_w * scale);
   int draw_h  = (int)(tex_h * scale);
   int x       = ((int)video_width  - draw_w) / 2;
   int y       = ((int)video_height - draw_h) / 2;

   gfx_display_draw_quad(p_disp, userdata, video_width, video_height,
         x, y, draw_w, draw_h, video_width, video_height, color, &tex);
}

/* HAKSWITCH TEST: rectangular (square-corner) frame around a box, drawn
 * as 4 thin quads - same technique ozone_draw_cursor_fallback already
 * uses for its selection outline. No rounded-corner asset needed. */
static void hakswitch_draw_box_border(gfx_display_t *p_disp, void *userdata,
      unsigned video_width, unsigned video_height,
      int x, int y, int w, int h, int thickness, float *color)
{
   gfx_display_draw_quad(p_disp, userdata, video_width, video_height,
...
/* HAKSWITCH TEST: dark gradient over the wallpaper - lighter at the top,
 * darker at the bottom, so the carousel/panel/HUD stay legible without
 * the background competing with them. gfx_display_draw_quad's "color"
 * is per-vertex, but with vertex generation left to the display driver
 * (coords.vertex == NULL) the winding order isn't ours to rely on -
 * stacking several bands with interpolated alpha gets the same visual
 * result without needing to know that order. */
static void hakswitch_draw_gradient_overlay(gfx_display_t *p_disp, void *userdata,
      unsigned video_width, unsigned video_height)
{
   static const int   bands      = 60;
   static const float top_alpha  = 0.10f;
   static const float bot_alpha  = 0.55f;
   int i;

   /* Bands must tile EXACTLY - no gaps, no overlaps. Rounding each
    * boundary independently (instead of a fixed height + fudge factor)
    * guarantees that: an off-by-one "+1" height previously made every
    * band overlap the next by a pixel, and since alpha blending stacks,
    * every one of those seams got double-darkened - a fine, perfectly
    * regular set of horizontal stripes across the whole screen. */
   for (i = 0; i < bands; i++)
   {
      static float band_color[16] = COLOR_HEX_TO_FLOAT(0x000000, 1.00f);
      float t       = (float)i / (float)(bands - 1);
      int   band_y0 = (int)(((int64_t)video_height * i) / bands);
      int   band_y1 = (int)(((int64_t)video_height * (i + 1)) / bands);

      gfx_display_set_alpha(band_color, top_alpha + (bot_alpha - top_alpha) * t);
      gfx_display_draw_quad(p_disp, userdata, video_width, video_height,
            0, band_y0, video_width, band_y1 - band_y0, video_width, video_height,
            band_color, NULL);
   }
}

/* "Contain" fit (never crops or distorts, letterboxes instead) - used
 * for logos inside a fixed-size card. */
static void hakswitch_draw_texture_contain(gfx_display_t *p_disp, void *userdata,
      unsigned video_width, unsigned video_height,
      int box_x, int box_y, int box_w, int box_h,
      uintptr_t tex, unsigned tex_w, unsigned tex_h, float *color)
{
   float scale = (tex_w && tex_h)
         ? MIN((float)box_w / tex_w, (float)box_h / tex_h)
         : 1.0f;
   int draw_w  = (int)(tex_w * scale);
   int draw_h  = (int)(tex_h * scale);
   int x       = box_x + (box_w - draw_w) / 2;
   int y       = box_y + (box_h - draw_h) / 2;

   gfx_display_draw_quad(p_disp, userdata, video_width, video_height,
         x, y, draw_w, draw_h, video_width, video_height, color, &tex);
}

/* HAKSWITCH TEST: per-game cover art, from consoles/<Name>/capas/, one
 * image per game named to match the ROM (any extension - the "capas"
 * folder is scanned once per game and matched by basename, unlike the
 * console logo/background folders which just take whatever single
 * file is in there). Cached by the game's full ROM path, since that's
 * already unique. */
typedef struct
{
   char path[1024];
   uintptr_t cover_tex;
   unsigned  cover_w, cover_h;
   /* HAKSWITCH TEST: optional gameplay screenshot, from the sibling
    * "screenshots" folder (same basename-match convention as covers).
    * Zero if the game has none - the info panel just hides that column. */
   uintptr_t screenshot_tex;
   unsigned  screenshot_w, screenshot_h;
   bool tried;
   /* HAKSWITCH TEST: metadata read from consoles/<Name>/metadata/<rom>.json
    * (same "match by basename" convention as the cover). "name" is the
    * clean display title - falls back to the ROM filename (extension
    * stripped) when there's no metadata file yet. */
   char name[256];
   char description[512];
   char year[16];
   char region[64];
} hakswitch_game_asset_t;
 ...
 * where it's called - unlike gfx_display_reset_textures_list, which
 * reads and decodes synchronously on the spot, blocking the whole
 * frame until it's done. That synchronous decode was confirmed (via a
 * diagnostic build that skipped all hakswitch image loading entirely
 * and ran freeze-free) to be the actual cause of the multi-second
 * freezes hit on Switch the first time a console/game list is shown.
 * By the time this callback fires, decode is already finished - the
 * only work left here, video_driver_texture_load's GPU upload, is
 ...
 * "year":"...","region":"..."} schema - not a general JSON parser,
 * just enough for this known shape (no escaped quotes expected). */
static void hakswitch_json_extract_string(const char *json,
      ...
      const char *roms_dir, const char *rom_filename)
{
   int i;
   char rom_path[1024];
   hakswitch_game_asset_t *slot = NULL;

   fill_pathname_join_special(rom_path, roms_dir, rom_filename, sizeof(rom_path));

   for (i = 0; i < hakswitch_game_asset_count; i++)
   {
      if (string_is_equal(hakswitch_game_assets[i].path, rom_path))
      {
         slot = &hakswitch_game_assets[i];
         break;
      }
   }

   if (!slot && hakswitch_game_asset_count < 64)
   {
      slot = &hakswitch_game_assets[hakswitch_game_asset_count++];
      strlcpy(slot->path, rom_path, sizeof(slot->path));
      slot->cover_tex = 0;
      slot->cover_w = slot->cover_h = 0;
      slot->screenshot_tex = 0;
      slot->screenshot_w = slot->screenshot_h = 0;
      slot->name[0] = slot->description[0] = slot->year[0] = slot->region[0] = '\0';
      slot->tried = false;
   }
...
/* HAKSWITCH TEST: generic placeholder art for a game with no matching
 * file in "capas" - loaded once and reused for every such game, so the
 * carousel never falls back to a plain colored box for a missing cover. */
typedef struct { uintptr_t tex; unsigned w, h; bool tried; } hakswitch_no_cover_t;
static hakswitch_no_cover_t hakswitch_no_cover;

static void hakswitch_load_no_cover(void)
{
   if (hakswitch_no_cover.tried)
      return;
   hakswitch_no_cover.tried = true;

   {
      char path[1024];
      fill_pathname_join_special(path, hakswitch_base_path, "src", sizeof(path));
      fill_pathname_join_special(path, path, "capa-nao-tem-jogo.png", sizeof(path));

#if !defined(HAKSWITCH_DIAG_NO_IMAGES)
      gfx_display_reset_textures_list(path, "",
            &hakswitch_no_cover.tex, TEXTURE_FILTER_MIPMAP_LINEAR,
            &hakswitch_no_cover.w, &hakswitch_no_cover.h);
#endif
   }
}

/* HAKSWITCH TEST: bottom HUD bar showing button hints (icon + label),
 * e.g. "(<-) (->) Navegar   (A) Selecionar" - icons come from
 * meu projeto/icons, pre-rasterized from SVG to PNG (rpng doesn't
 * read SVG) and loaded like any other texture asset here. */
typedef struct
{
   uintptr_t tex;
   unsigned  w, h;
   bool tried;
} hakswitch_hud_icon_t;
...
static hakswitch_hud_icon_t hakswitch_hud_y;

static void hakswitch_load_hud_icon(hakswitch_hud_icon_t *icon, const char *filename)
{
   char path[1024];
   ...
   {
      char icons_dir[1024];
      fill_pathname_join_special(icons_dir, hakswitch_base_path, "icons", sizeof(icons_dir));
      fill_pathname_join_special(path, icons_dir, filename, sizeof(path));
   }
#if !defined(HAKSWITCH_DIAG_NO_IMAGES)
   gfx_display_reset_textures_list(path, "",
         &icon->tex, TEXTURE_FILTER_MIPMAP_LINEAR, &icon->w, &icon->h);
#endif
}

/* A hint is 1-2 icons (e.g. left+right for "Navegar") followed by a
 * label - laid out left to right and the whole row centered. */
typedef struct
{
   hakswitch_hud_icon_t *icon1;
      ...
      void *userdata, unsigned video_width, unsigned video_height, bool inside_console)
{
   ...
   int padding      = 16;
   int bar_h        = icon_size + padding * 2;
   int bar_y        = (int)video_height - bar_h;
   int y            = bar_y + padding;
   ...
   gfx_display_draw_quad(p_disp, userdata, video_width, video_height,
         0, bar_y, video_width, bar_h, video_width, video_height,
         hakswitch_hud_bg, NULL);
   gfx_display_draw_quad(p_disp, userdata, video_width, video_height,
   ...
   for (i = 0; i < n_hints; i++)
   {
      hint_label_w[i] = font_driver_get_message_width(
            ozone->fonts.entries_label.font, hints[i].label, strlen(hints[i].label), text_scale);

      total_w += icon_size + small_gap;
      if (hints[i].icon2)
         total_w += icon_size + small_gap;
      total_w += (int)hint_label_w[i];
      if (i > 0)
         total_w += group_gap;
   }

   x      = (int)video_width / 2 - total_w / 2;
   ...
   for (i = 0; i < n_hints; i++)
   {
      ...
      if (hints[i].icon1->tex)
         hakswitch_draw_texture_contain(p_disp, userdata, video_width, video_height,
               x, y, icon_size, icon_size,
               hints[i].icon1->tex, hints[i].icon1->w, hints[i].icon1->h, hakswitch_hud_white);
      x += icon_size + small_gap;

      if (hints[i].icon2)
      {
         if (hints[i].icon2->tex)
            hakswitch_draw_texture_contain(p_disp, userdata, video_width, video_height,
                  x, y, icon_size, icon_size,
                  hints[i].icon2->tex, hints[i].icon2->w, hints[i].icon2->h, hakswitch_hud_white);
         x += icon_size + small_gap;
      }

      gfx_display_draw_text(ozone->fonts.entries_label.font, hints[i].label,
            (float)x, (float)text_y + ozone->fonts.entries_label.line_centre_offset,
            video_width, video_height, COLOR_TEXT_ALPHA(0xFFFFFF00, 255),
            TEXT_ALIGN_LEFT, text_scale, false, 1.0f, false);
      x += (int)hint_label_w[i];
   }
}
 ...
 * corner, matching the reference this project is modeled after: the
 * paused game frame stays visible (dimmed) behind it, instead of
 * being replaced by a full-screen menu the way stock Ozone's Quick
 * Menu is. */
static void ozone_draw_hakswitch_quick_menu(
      ozone_handle_t *ozone,
      gfx_display_t *p_disp,
      void *userdata,
      unsigned video_width,
      ...
      file_list_t *selection_buf)
{
   ...
   gfx_display_draw_quad(p_disp, userdata, video_width, video_height,
         ...
         video_width, video_height, hakswitch_qm_box_bg, NULL);
   hakswitch_draw_box_border(p_disp, userdata, video_width, video_height,
         box_x, box_y, box_w, box_h, border_px, hakswitch_qm_border);

   for (i = 0; i < entries_count; i++)
   {
      menu_entry_t entry;
      const char  *label;
      bool         is_selected = (i == (size_t)selection);
      int          row_y       = box_y + pad + (int)i * row_h;

      MENU_ENTRY_INITIALIZE(entry);
      entry.flags |= MENU_ENTRY_FLAG_RICH_LABEL_ENABLED | MENU_ENTRY_FLAG_LABEL_ENABLED;
      menu_entry_get(&entry, 0, (unsigned)i, selection_buf, true);
      label = (*entry.rich_label) ? entry.rich_label : entry.path;

      if (is_selected)
         gfx_display_draw_quad(p_disp, userdata, video_width, video_height,
               box_x + (int)(pad * 0.4f), row_y, box_w - (int)(pad * 0.8f), row_h,
               video_width, video_height, hakswitch_qm_highlight, NULL);

      gfx_display_draw_text(
            ozone->fonts.entries_label.font,
            label,
            (float)(box_x + pad),
            (float)(row_y + row_h / 2) + ozone->fonts.entries_label.line_centre_offset,
            video_width, video_height,
            is_selected ? COLOR_TEXT_ALPHA(0x1b0f3a00, 255) : COLOR_TEXT_ALPHA(0xFFFFFF00, 255),
            TEXT_ALIGN_LEFT,
            1.0f,
            false, 1.0f, false);
   }
 ...
 * Draws a colored square per entry (bigger + highlighted for the
 * selected one) with its label underneath, wrapping around instead
 * of a vertical scrolling list. Reuses the existing selection_ptr /
 * up-down navigation already wired generically by the menu system -
 * only the rendering changes here. */
static void ozone_draw_hakswitch_carousel(
      ozone_handle_t *ozone,
      gfx_display_t *p_disp,
      void *userdata,
      unsigned video_width,
...
{
   int slot;
   int entries_end = selection_buf ? (int)selection_buf->size : 0;
   int center_x     = (int)video_width  / 2;

   /* HAKSWITCH TEST: only the console list (root) has per-item logo/
    * background art - a console's game list (inside "roms") doesn't.
    * Layout proportions differ between the two screens (see below) -
    * the root screen keeps its previously-approved Batocera-derived
    * numbers untouched, only the in-console game list is re-tuned to
    * match the user's reference mockup. */
   bool at_console_root = string_is_equal(hakswitch_current_path, hakswitch_root_path);

   /* HAKSWITCH TEST: root screen keeps the original Batocera/
    * EmulationStation "systemCarousel" proportions (band 1x0.2325,
    * position 0x0.38375, logo 0.25x0.155, scale 1.2). The in-console
    * game list instead copies the measured proportions of the user's
    * reference image: selected cover is the largest element on
    * screen (~2x the info panel's height), cards sit close together
    * (small gap), no band behind them. */
   int band_h       = at_console_root
         ? (int)(video_height * 0.2325f)
         : (int)(video_height * 0.37f);
   int band_y0      = at_console_root
         ? (int)(video_height * 0.38375f)
         : (int)(video_height * 0.2825f);
   int center_y     = band_y0 + band_h / 2;
   int spacing      = at_console_root
         ? (int)(video_width * 0.25f)
         : (int)(video_width * 0.19f);
   /* In-console cards sit almost edge to edge (small gap) instead of
    * the root screen's wide gap. */
   int item_w       = at_console_root
         ? (int)(spacing * 0.78f)
         : (int)(spacing * 0.94f);
   int item_h       = at_console_root
         ? (int)(video_height * 0.155f)
         : (int)(band_h / 1.2f);
   /* HAKSWITCH TEST: slide animation - whenever selection changes we
    * snap this offset to a full slot-width displacement and animate it
    * back to 0, so the carousel visibly slides into place instead of
    * instantly popping to the new selection. */
   static float hakswitch_slide_offset       = 0.0f;
   static int   hakswitch_last_selection     = -1;
   static float hakswitch_box_card[16]         = COLOR_HEX_TO_FLOAT(0x2B2B2B, 1.00f);
   static float hakswitch_box_card_border[16]  = COLOR_HEX_TO_FLOAT(0x555555, 1.00f);
   static float hakswitch_border[16]           = COLOR_HEX_TO_FLOAT(0x6CC6FF, 1.00f);
   static float hakswitch_glow[16]             = COLOR_HEX_TO_FLOAT(0x6CC6FF, 0.35f);
   /* HAKSWITCH TEST: fade-in for the background/logo art - restarted
    * (0 -> 1 over ANIMATION_CURSOR_DURATION) whenever the art actually
    * changes: a new console centered at root, or entering/leaving a
    * console's game list. Applied as an alpha multiplier on the art
    * quads only (cards/text stay instant). */
   static float hakswitch_art_fade           = 1.0f;
   static bool  hakswitch_last_at_root       = true;

   if (entries_end == 0)
      return;

   if ((int)selection != hakswitch_last_selection)
   {
      if (hakswitch_last_selection != -1)
      {
         int diff = (int)selection - hakswitch_last_selection;

         /* Normalize wraparound jumps (e.g. last item -> first item)
          * down to a simple +1/-1 step, so the slide direction still
          * matches the direction the user actually pressed. */
         if (diff > entries_end / 2)
            diff -= entries_end;
         else if (diff < -(entries_end / 2))
            diff += entries_end;

         {
            gfx_animation_ctx_entry_t anim_entry;
            uintptr_t anim_tag = (uintptr_t)&hakswitch_slide_offset;

            gfx_animation_kill_by_tag(&anim_tag);
            hakswitch_slide_offset = (float)(diff * spacing);

            anim_entry.cb           = NULL;
            anim_entry.duration     = ANIMATION_CURSOR_DURATION;
            anim_entry.easing_enum  = OZONE_EASING_ALPHA;
            anim_entry.subject      = &hakswitch_slide_offset;
            anim_entry.tag          = anim_tag;
            anim_entry.target_value = 0.0f;
            anim_entry.userdata     = NULL;

            gfx_animation_push(&anim_entry);
         }

         /* Background art only changes when a new console gets
          * centered at root - fade it in each time that happens. */
         if (at_console_root)
         {
            gfx_animation_ctx_entry_t fade_entry;
            uintptr_t fade_tag = (uintptr_t)&hakswitch_art_fade;

            gfx_animation_kill_by_tag(&fade_tag);
            hakswitch_art_fade = 0.0f;

            fade_entry.cb           = NULL;
            fade_entry.duration     = ANIMATION_CURSOR_DURATION;
            fade_entry.easing_enum  = OZONE_EASING_ALPHA;
            fade_entry.subject      = &hakswitch_art_fade;
            fade_entry.tag          = fade_tag;
            fade_entry.target_value = 1.0f;
            fade_entry.userdata     = NULL;

            gfx_animation_push(&fade_entry);
         }
      }
      hakswitch_last_selection = (int)selection;
   }

   /* Screen transition (entering/leaving a console) also fades the art
    * in, same mechanism as above. */
   if (at_console_root != hakswitch_last_at_root)
   {
      gfx_animation_ctx_entry_t fade_entry;
      uintptr_t fade_tag = (uintptr_t)&hakswitch_art_fade;
      ...
      hakswitch_last_at_root = at_console_root;
   }

   /* Full-screen background art behind everything, for whichever
    * console is currently centered/selected - drawn before the band
    * so the band stays a translucent overlay on top of it. Only at
    * the console-list level; a game list has no per-item background. */
   if (at_console_root)
   {
      static float hakswitch_art_white[16] = COLOR_HEX_TO_FLOAT(0xFFFFFF, 1.00f);
      size_t sel_idx = (size_t)selection;
      menu_entry_t sel_entry;
      const char *sel_label;
      hakswitch_console_asset_t *assets;

      MENU_ENTRY_INITIALIZE(sel_entry);
      sel_entry.flags |= MENU_ENTRY_FLAG_RICH_LABEL_ENABLED | MENU_ENTRY_FLAG_LABEL_ENABLED;
      menu_entry_get(&sel_entry, 0, (unsigned)sel_idx, selection_buf, true);
      sel_label = (*sel_entry.rich_label) ? sel_entry.rich_label : sel_entry.path;
      ...
      if (assets && assets->bg_tex)
      {
         ...
         hakswitch_draw_texture_cover(p_disp, userdata, video_width, video_height,
               assets->bg_tex, assets->bg_w, assets->bg_h, hakswitch_art_white);
      }
   }
   else
   {
      ...
      strlcpy(console_dir, hakswitch_current_path, sizeof(console_dir));
      /* hakswitch_current_path is ".../<ConsoleName>/roms" here - strip
       * "roms" to get ".../<ConsoleName>[/]", then its basename. */
      fill_pathname_parent_dir(console_dir, console_dir, sizeof(console_dir));
      {
         size_t clen = strlen(console_dir);
         if (clen > 0 && (console_dir[clen - 1] == '/' || console_dir[clen - 1] == '\\'))
            console_dir[clen - 1] = '\0';
      ...
      assets = hakswitch_get_console_assets(path_basename(console_dir));

      if (assets)
      {
         gfx_display_set_alpha(hakswitch_dim_black, 1.00f * hakswitch_art_fade);
         gfx_display_draw_quad(p_disp, userdata, video_width, video_height,
         ...
         if (assets->logo_tex)
         {
            int logo_box_w = (int)(video_width  * 0.22f);
            int logo_box_h = (int)(video_height * 0.16f);

            gfx_display_set_alpha(hakswitch_art_white, hakswitch_art_fade);
            hakswitch_draw_texture_contain(p_disp, userdata, video_width, video_height,
                  center_x - logo_box_w / 2, (int)(video_height * 0.04f),
                  logo_box_w, logo_box_h,
                  assets->logo_tex, assets->logo_w, assets->logo_h, hakswitch_art_white);
         }
      }
   }

   hakswitch_draw_gradient_overlay(p_disp, userdata, video_width, video_height);

   /* HAKSWITCH TEST: the root console-list screen keeps its translucent
    * band behind the carousel (previously approved look). The in-console
    * game list does NOT get one - per the reference mockup, the wallpaper
    * stays visible behind the cards, with only the dark gradient above
    * for legibility. */
   if (at_console_root)
   {
      static float hakswitch_band_bg[16] = COLOR_HEX_TO_FLOAT(0xFFFFFF, 0.847f);
      gfx_display_draw_quad(p_disp, userdata, video_width, video_height,
            0, band_y0, video_width, band_h, video_width, video_height,
            hakswitch_band_bg, NULL);
   }

   /* HAKSWITCH TEST: the selected card is now the biggest element on
    * screen and can overlap its neighbors' slots (small gap) - drawn in
    * plain -2..2 order, a neighbor drawn after it would paint over its
    * edge. Draw order below is -2,-1,1,2,0 instead, so the selected
    * slot is always painted last and stays on top. */
   for (slot = -2; slot <= 2; slot++)
   {
      int slot_order[5] = { -2, -1, 1, 2, 0 };
      int draw_slot        = slot_order[slot + 2];
      size_t idx          = (size_t)(((int)selection + draw_slot + entries_end * 4) % entries_end);
      bool   is_selected   = (draw_slot == 0);
      float  logo_scale    = is_selected ? 1.2f * hakswitch_select_pulse_scale : 1.0f;
      int    w             = (int)(item_w * logo_scale);
      int    h             = (int)(item_h * logo_scale);
      int    x;
      int    y             = center_y - h / 2;
      menu_entry_t entry;
      const char *label;
      ...
      MENU_ENTRY_INITIALIZE(entry);
      entry.flags |= MENU_ENTRY_FLAG_RICH_LABEL_ENABLED | MENU_ENTRY_FLAG_LABEL_ENABLED;
      menu_entry_get(&entry, 0, (unsigned)idx, selection_buf, true);
      label = (*entry.rich_label) ? entry.rich_label : entry.path;

      if (at_console_root)
      {
         hakswitch_console_asset_t *assets = hakswitch_get_console_assets(label);
         if (assets)
         {
            art_tex = assets->logo_tex;
            art_w   = assets->logo_w;
            art_h   = assets->logo_h;
         }
      }
      else
      {
         ...
         /* No matching cover file for this game - use the generic
          * placeholder instead of falling back to a plain colored box. */
         if (!art_tex)
         {
            hakswitch_load_no_cover();
            if (hakswitch_no_cover.tex)
            {
               art_tex = hakswitch_no_cover.tex;
               art_w   = hakswitch_no_cover.w;
               art_h   = hakswitch_no_cover.h;
            }
         }
      }

      /* HAKSWITCH TEST: the card's WIDTH follows the cover's own aspect
       * ratio at a fixed height - a horizontal box gets a wider card, a
       * vertical one a narrower card, a square one in between, instead
       * of forcing every cover into one fixed portrait-shaped box
       * (which either crops or letterboxes non-portrait art). Clamped
       * so a wildly extreme aspect ratio can't blow out the layout. */
      if (art_tex && !at_console_root && art_h > 0)
      {
         float aspect = (float)art_w / (float)art_h;
         int   fit_w  = (int)(h * aspect);
         int   min_w  = (int)(item_w * 0.6f);
         int   max_w  = (int)(item_w * 2.4f);

         if (fit_w < min_w) fit_w = min_w;
         if (fit_w > max_w) fit_w = max_w;
         w = fit_w;
      }
      x = center_x + draw_slot * spacing - w / 2 + (int)hakswitch_slide_offset;

      if (art_tex && !at_console_root)
      {
         /* Game cover: "contain" fit - the whole cover is always
          * visible, never cropped. With the card width now matched to
          * the cover's own aspect ratio above, this fits edge to edge
          * for most covers; the dark backdrop only shows if the aspect
          * ratio got clamped. */
         static float hakswitch_cover_dim[16] = COLOR_HEX_TO_FLOAT(0xFFFFFF, 0.65f);

         if (is_selected)
         {
            /* Cheap "glow": a slightly larger quad of the same color
             * behind the cover, at low alpha - not a real blur, but
             * gives a soft halo without needing a baked texture asset. */
            int glow_pad = (int)(video_height * 0.010f);
            gfx_display_draw_quad(p_disp, userdata, video_width, video_height,
                  x - glow_pad, y - glow_pad, w + glow_pad * 2, h + glow_pad * 2,
                  video_width, video_height, hakswitch_glow, NULL);
         }

         gfx_display_draw_quad(p_disp, userdata, video_width, video_height,
               x, y, w, h, video_width, video_height, hakswitch_box_card, NULL);

         hakswitch_draw_texture_contain(p_disp, userdata, video_width, video_height,
               x, y, w, h,
               art_tex, art_w, art_h, is_selected ? ozone->pure_white : hakswitch_cover_dim);

         if (is_selected)
         {
            int border_px = MAX(2, (int)(video_height * 0.0026f));
            hakswitch_draw_box_border(p_disp, userdata, video_width, video_height,
                  x, y, w, h, border_px, hakswitch_border);

            /* "Loading" spinner: shown for the same dead window the
             * actual game launch is deliberately delayed for (see
             * hakswitch_check_pending_launch, menu_cbs_ok.c - the
             * "launch" sound needs time to actually play before
             * content-load tears the audio driver down), so that
             * window doesn't read as a freeze. Small dots chasing
             * around a ring, overlaid on the cover's bottom edge - no
             * texture needed, same approach as the rest of this
             * carousel. */
            if (hakswitch_is_launch_pending())
            {
               int   spin_cx     = x + w / 2;
               int   spin_cy     = y + h - (int)(video_height * 0.045f);
               int   spin_radius = (int)(video_height * 0.016f);
               int   dot_size    = MAX(2, (int)(video_height * 0.0055f));
               int   num_dots    = 8;
               ...
               for (di = 0; di < num_dots; di++)
               {
                  ...
                  memcpy(dot_color, hakswitch_border, sizeof(dot_color));
         ...
         /* Console logo: never crop or distort, letterbox within the
          * card instead. */
         hakswitch_draw_texture_contain(p_disp, userdata, video_width, video_height,
               x, y, w, h,
               art_tex, art_w, art_h, ozone->pure_white);
      else
      {
         /* No art at all (no logo/cover found) - dark card with a
          * thin border instead of a flat solid color, matching the
          * covers' card look. Selected still gets the glow/blue
          * border treatment so it reads as highlighted either way. */
         if (is_selected)
         {
            int glow_pad = (int)(video_height * 0.010f);
            gfx_display_draw_quad(p_disp, userdata, video_width, video_height,
         ...
         {
            int border_px = is_selected ? MAX(2, (int)(video_height * 0.0026f)) : 1;
            hakswitch_draw_box_border(p_disp, userdata, video_width, video_height,
                  x, y, w, h, border_px,
                  is_selected ? hakswitch_border : hakswitch_box_card_border);
         }
      }

      /* Logos/covers already show what the item is, so skip the
       * redundant text label - only plain-color cards need their name
       * drawn, and it goes INSIDE the card (like the logos have their
       * name baked into the image) rather than floating below it. Long
       * game/console names get shrunk to fit the card width instead of
       * overflowing into neighboring cards. */
      if (!art_tex)
      {
         char display_label[512];
         float desired_scale = is_selected ? 1.3f : 1.0f;
         float fit_scale     = desired_scale;
         int   max_text_w    = (int)(w * 0.88f);
         unsigned measured_w;

         /* Games are bare ROM filenames ("Sonic.md") - show the name
          * without the ROM extension. Console names have no extension
          * to begin with, so this is a no-op for those. */
         strlcpy(display_label, label, sizeof(display_label));
         path_remove_extension(display_label);

         measured_w = font_driver_get_message_width(
               ozone->fonts.entries_label.font, display_label, strlen(display_label), 1.0f);

         if (measured_w > 0 && (int)(measured_w * desired_scale) > max_text_w)
            fit_scale = (float)max_text_w / (float)measured_w;

         gfx_display_draw_text(
               ozone->fonts.entries_label.font,
               display_label,
               (float)(center_x + draw_slot * spacing) + hakswitch_slide_offset,
               (float)(y + h / 2) + ozone->fonts.entries_label.line_centre_offset,
               video_width, video_height,
               COLOR_TEXT_ALPHA(0xFFFFFF00, 255),
               TEXT_ALIGN_CENTER,
               fit_scale,
               false, 1.0f, false);
      }
   ...
   /* HAKSWITCH TEST: below the carousel, inside a console's game list -
    * a dedicated bordered panel (never text loose over the wallpaper)
    * with the selected game's metadata/title/description/screenshot,
    * read from its metadata json (see hakswitch_get_game_cover, which
    * loads the cover texture, screenshot and this metadata together).
    * Going back up happens via the B button (see MENU_ACTION_CANCEL
    * handling in ozone_parse_menu_entry_action) - no on-screen button. */
   if (!at_console_root)
   {
      size_t sel_idx = (size_t)selection;
      menu_entry_t sel_entry;
      const char *sel_label;
      hakswitch_game_asset_t *sel_game;

      static float hakswitch_panel_bg[16]     = COLOR_HEX_TO_FLOAT(0x111111, 0.85f);
      static float hakswitch_panel_border[16] = COLOR_HEX_TO_FLOAT(0xFFFFFF, 0.10f);

      /* HAKSWITCH TEST: sized to be a discreet HUD-like strip, not a
       * window - roughly half the selected cover's height, per the
       * reference mockup (selected cover ~2x the panel's height). */
      int panel_w    = (int)(video_width  * 0.84f);
      int panel_h    = (int)(video_height * 0.185f);
      int panel_x    = center_x - panel_w / 2;
      int panel_y    = band_y0 + band_h + (int)(video_height * 0.035f);
      /* HAKSWITCH TEST: ONE padding value, in pixels, reused identically
       * on every edge (top/bottom/left/right) - like a single CSS
       * "padding" rule - instead of separate x/y percentages that drifted
       * out of sync with each other. */
      int pad        = (int)(panel_h * 0.16f);
      int left_w     = (int)(panel_w * 0.16f);
      /* Screenshot is a small complement, not competing with the cover -
       * sized to exactly fill the space left after top/bottom padding,
       * so its own margin matches the text's padding instead of being
       * separately (and inconsistently) centered. */
      int shot_h     = panel_h - pad * 2;
      int shot_w     = (int)(shot_h * 16.0f / 9.0f);
      ...
      sel_game = hakswitch_get_game_cover(hakswitch_current_path, sel_label);

      gfx_display_draw_quad(p_disp, userdata, video_width, video_height,
            panel_x, panel_y, panel_w, panel_h, video_width, video_height,
            hakswitch_panel_bg, NULL);
      hakswitch_draw_box_border(p_disp, userdata, video_width, video_height,
            panel_x, panel_y, panel_w, panel_h, 1, hakswitch_panel_border);

      if (sel_game)
      {
         bool has_shot        = sel_game->screenshot_tex != 0;
         int  center_col_x    = panel_x + pad + left_w + pad;
         int  center_col_w    = panel_w - pad * (has_shot ? 3 : 2) - left_w - (has_shot ? shot_w : 0);
         /* HAKSWITCH TEST: text y-coordinates aren't a "top edge" in
          * this font API - two different fonts (bold title vs regular
          * metadata) given the same raw y land at visibly different
          * heights because their internal ascent/line-height metrics
          * differ. Anchoring every row through its own font's
          * line_centre_offset (the same trick already used to center
          * card labels elsewhere in this file) makes each font's text
          * line up at the same visual top, regardless of font. */
         int  row_top         = panel_y + pad;
         char title_noext[512];
         const char *display_title;

         if (*sel_game->name)
            display_title = sel_game->name;
         else
         {
            strlcpy(title_noext, sel_label, sizeof(title_noext));
            path_remove_extension(title_noext);
            display_title = title_noext;
         }

         /* HAKSWITCH TEST: a game with no metadata.json has nothing but
            its filename to show - the left-aligned/top-anchored layout
            below reads as broken (title floating alone near the top)
            when there's no year/region/description around it. In that
            case just center the title in the whole panel instead. */
         if (!*sel_game->year && !*sel_game->region && !*sel_game->description)
            gfx_display_draw_text(
                  ozone->fonts.entries_label.font, display_title,
                  (float)(panel_x + panel_w / 2),
                  (float)(panel_y + panel_h / 2 + ozone->fonts.entries_label.line_centre_offset),
                  video_width, video_height,
                  COLOR_TEXT_ALPHA(0xFFFFFF00, 255),
                  TEXT_ALIGN_CENTER, 1.1f, false, 1.0f, false);
         else
         {
            /* Left column: plain metadata lines - no calendar/people icon
               assets exist yet, so this stays text-only for now. */
            if (*sel_game->year)
            {
               char line[64];
               snprintf(line, sizeof(line), "%s", sel_game->year);
               gfx_display_draw_text(
                     ozone->fonts.entries_sublabel.font, line,
                     (float)(panel_x + pad),
                     (float)(row_top + ozone->fonts.entries_sublabel.line_height / 2
                           + ozone->fonts.entries_sublabel.line_centre_offset),
                     video_width, video_height,
                     COLOR_TEXT_ALPHA(0xCCCCCC00, 255),
                     TEXT_ALIGN_LEFT, 1.0f, false, 1.0f, false);
            }
            if (*sel_game->region)
            {
               gfx_display_draw_text(
                     ozone->fonts.entries_sublabel.font, sel_game->region,
                     (float)(panel_x + pad),
                     (float)(row_top + ozone->fonts.entries_sublabel.line_height
            ...
            /* Center column: title + description, left-aligned inside
               the panel instead of centered on the whole screen. */
            gfx_display_draw_text(
                  ozone->fonts.entries_label.font, display_title,
                  (float)center_col_x,
                  (float)(row_top + ozone->fonts.entries_label.line_height / 2
                        + ozone->fonts.entries_label.line_centre_offset),
                  video_width, video_height,
                  COLOR_TEXT_ALPHA(0xFFFFFF00, 255),
                  TEXT_ALIGN_LEFT, 1.1f, false, 1.0f, false);

            if (*sel_game->description)
            {
               char wrapped[1024];
               int max_chars  = center_col_w / (int)ozone->fonts.entries_sublabel.glyph_width;
               int desc_top   = row_top + (int)(ozone->fonts.entries_label.line_height * 1.3f);
               char *line;
               char *saveptr = NULL;
               int line_i = 0;

               (ozone->word_wrap)(wrapped, sizeof(wrapped),
                     sel_game->description, strlen(sel_game->description),
                     max_chars, ozone->fonts.entries_sublabel.wideglyph_width, 0);

               for (line = strtok_r(wrapped, "\n", &saveptr);
                     line && line_i < 3;
                     line = strtok_r(NULL, "\n", &saveptr), line_i++)
               {
                  gfx_display_draw_text(
                        ozone->fonts.entries_sublabel.font, line,
                        (float)center_col_x,
                        (float)(desc_top + line_i * ozone->fonts.entries_sublabel.line_height
                              + ozone->fonts.entries_sublabel.line_height / 2
                              + ozone->fonts.entries_sublabel.line_centre_offset),
                        video_width, video_height,
                        COLOR_TEXT_ALPHA(0xAAAAAA00, 255),
                        TEXT_ALIGN_LEFT, 1.0f, false, 1.0f, false);
               }
            }

            /* Right column: gameplay screenshot, if the game has one -
               just hidden (no placeholder) when it doesn't. Top-anchored
               at the same "pad" as the text, sized to exactly reach
               "pad" from the bottom too - same margin on every side. */
            if (has_shot)
               hakswitch_draw_texture_contain(p_disp, userdata, video_width, video_height,
                     panel_x + panel_w - pad - shot_w,
                     panel_y + pad,
                     shot_w, shot_h,
                     sel_game->screenshot_tex, sel_game->screenshot_w, sel_game->screenshot_h,
                     ozone->pure_white);
         }
      }
   }

   /* HAKSWITCH TEST: button-hint HUD - fewer hints at the console list
    * (root), more once inside a console's game list. */
   hakswitch_draw_hud_bar(ozone, p_disp, userdata, video_width, video_height, !at_console_root);
}
      ...
      enum menu_action action)
{
    ...
    * ozone's normal left/right, which try to focus the (hidden) sidebar.
    * Must happen before 'new_action' picks up 'action' below, since the
         ...
         && menu_stack_size <= 1)
   {
      ...
      if (inside_console && action == MENU_ACTION_CANCEL)
      {
         /* B button - go back up to the console list. */
         char console_dir[1024];

         fill_pathname_parent_dir(hakswitch_current_path,
               hakswitch_current_path, sizeof(hakswitch_current_path));
         /* hakswitch_current_path is now the console's own folder
          * (".../consoles/<Name>/") - one level short of root. Captured
          * here (trailing slash stripped below) so DISPLAYLIST_MAIN_MENU's
          * restore-by-path match (menu_displaylist.c, case
          * DISPLAYLIST_MAIN_MENU) can find this console's entry once the
          * root list rebuilds just below - the same "match by full path,
          * not index" mechanism that already restores the right game
          * inside a console, reused here instead of a separate
          * in-memory index. That index (hakswitch_root_selection, since
          * removed) never survived "Close Content" restarting the whole
          * process on Switch - a plain static, reset to 0 on the fresh
          * process, silently landing on whichever console sorts first
          * instead of the one just left. */
         strlcpy(console_dir, hakswitch_current_path, sizeof(console_dir));
         {
            size_t clen = strlen(console_dir);
            if (clen > 0 && (console_dir[clen - 1] == '/' || console_dir[clen - 1] == '\\'))
         ...
         fill_pathname_parent_dir(hakswitch_current_path,
               hakswitch_current_path, sizeof(hakswitch_current_path));
         /* fill_pathname_parent_dir keeps the trailing slash, but
          * hakswitch_root_path has none - strip it so the "are we at
          * root" string comparisons elsewhere actually match. */
         {
            size_t plen = strlen(hakswitch_current_path);
            if (plen > 0 && (hakswitch_current_path[plen - 1] == '/'
                     || hakswitch_current_path[plen - 1] == '\\'))
               hakswitch_current_path[plen - 1] = '\0';
         }
         menu_st->flags |= MENU_ST_FLAG_ENTRIES_NEED_REFRESH;
         action           = MENU_ACTION_NOOP;
         /* Re-purposed to restore the console just left, instead of
          * cleared - see the comment above console_dir. */
         strlcpy(hakswitch_last_played_path, console_dir, sizeof(hakswitch_last_played_path));
...
/* HAKSWITCH TEST: our own asset caches (console logos/backgrounds, game
 * covers/screenshots, HUD button icons, the no-cover placeholder) each
 * load their texture exactly once per process and never again (gated
 * by a "tried" flag / cache count) - reasonable for assets that never
 * change on disk mid-session, but wrong across a context reset (e.g.
 * closing a game's content), which tears down and reinitializes the
 * whole GPU texture pool. Without this, every one of our cached texture
 * handles goes stale and silently renders as black. Hooked into the
 * same ozone_context_reset() that already reloads Ozone's own textures
 * after such a reset - forgetting everything here just makes it lazily
 * reload next time each asset is actually drawn. */
static void hakswitch_invalidate_asset_caches(void)
{
   /* Resetting the counts is enough - the lookup functions only ever
    * search indices below the count, so old (now-stale) slots simply
    * stop being found and get transparently replaced as the caches
    * refill; a fresh slot always starts with tried = false anyway. */
   hakswitch_console_asset_count = 0;
   hakswitch_game_asset_count    = 0;

   hakswitch_hud_left.tried  = false;
   hakswitch_hud_right.tried = false;
   hakswitch_hud_a.tried     = false;
   hakswitch_hud_b.tried     = false;
   hakswitch_hud_x.tried     = false;
   hakswitch_hud_y.tried     = false;

   hakswitch_no_cover.tried = false;
}
...
static void ozone_context_reset(void *data, bool is_threaded)
{
   ...
   if (ozone)
   {
      ...
      hakswitch_invalidate_asset_caches();
   }
...
static void ozone_frame(void *data, video_frame_info_t *video_info)
{
   ...
   /* HAKSWITCH TEST: force a distinct background color, unrelated to any
    * theme, to prove we have real control over the visual style and
    * aren't just reusing Ozone's stock look - but only on our own
    ...
    * from the current list's own contents - stateless and recomputed
    * every frame, same philosophy as hakswitch_on_main_menu above, so
   ...
   if (!hakswitch_on_main_menu && MENU_LIST_GET_STACK_SIZE(menu_list, 0) > 1)
   {
      ...
      if (hakswitch_qm_list && hakswitch_qm_list->size > 0)
      {
         menu_file_list_cbs_t *hakswitch_qm_cbs =
    ...
    * gameplay starts (ozone_frame stops running then anyway). Stopped
    * rather than paused when leaving: the mixer API here doesn't have
    * a real pause, and hakswitch_ensure_bgm_playing already treats
   ...
   /* Header, footer */
   if (!hakswitch_on_main_menu && !hakswitch_on_quick_menu)
   ...
   /* Menu entries */
   /* HAKSWITCH TEST: the stock scissor rect reserves the old sidebar's
    * space on the left (still nonzero even though the sidebar itself
    * is hidden), which was clipping away the carousel's leftmost
    * square. Skip it on our carousel screen; keep it everywhere else. */
   if (!hakswitch_on_main_menu && !hakswitch_on_quick_menu && p_disp->dispctx && p_disp->dispctx->scissor_begin)
   ...
   /* Current list */
   /* HAKSWITCH TEST: draw the infinite horizontal carousel only on our
    * folder screen; every other tab keeps the stock vertical list. */
   if (hakswitch_on_main_menu)
      ozone_draw_hakswitch_carousel(ozone,
            p_disp,
            userdata,
            video_width,
            video_height,
   ...
   else if (hakswitch_on_quick_menu)
      ozone_draw_hakswitch_quick_menu(ozone,
            p_disp,
            userdata,
            video_width,
            video_height,
   ...
   /* Old list */
   if (!hakswitch_on_main_menu && !hakswitch_on_quick_menu && (ozone->flags & OZONE_FLAG_DRAW_OLD_LIST))
      ozone_draw_entries(ozone, icons_tex,
            p_disp,
            p_anim,
            settings,
