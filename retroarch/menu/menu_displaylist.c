...
/* HAKSWITCH TEST: current folder shown by our carousel main menu, and
 * the root it must not navigate above. Not static - menu_cbs_ok.c
 * reads/writes hakswitch_current_path when the user enters/exits a
 * folder (see action_ok_hakswitch_enter_dir / action_ok_hakswitch_go_up).
 * Left empty here and filled in at runtime by hakswitch_init_paths()
 * (below) - a hardcoded absolute path only ever worked on one PC. */
char hakswitch_base_path[1024]    = "";
char hakswitch_root_path[1024]    = "";
char hakswitch_current_path[1024] = "";
/* Reserved user mixer slot (of the 16 in AUDIO_MIXER_MAX_STREAMS) for
 * the preloaded "launch game" sound - kept in sync with the same
 * #define in menu_cbs_ok.c, which is what actually fires it. */
#define HAKSWITCH_LAUNCH_SFX_SLOT 0
/* Full path of whatever entry should be re-selected the next time the
 * list for hakswitch_current_path rebuilds - checked once by
 * DISPLAYLIST_MAIN_MENU below, whether that list is a console's game
 * list (set to a ROM path by action_ok_hakswitch_open_file, so
 * returning from "Close Content" re-selects that same game) or the
 * root console list (set to a console's folder path by the "go back
 * up" B-button handler in ozone.c, so going up reselects the console
 * just left instead of resetting to whichever one sorts first). Not
 ...
 * hakswitch_current_path/hakswitch_last_played_path don't survive
 * "Close Content" the way they do on PC (same process the whole
 * time). This file bridges that gap: written by
 * action_ok_hakswitch_open_file right before launching content, read
 * back here (once, at process start) so we land back on the exact
 * folder + game instead of the console root. Consumed once - deleted
 * right after being read, so a session that never launches a game
 * doesn't keep re-jumping to a stale prior one. */
static void hakswitch_load_last_played_state(void)
{
   ...
   if (fgets(line, sizeof(line), f))
   {
      ...
      if (len > 0 && line[len - 1] == '\n')
         line[len - 1] = '\0';
      strlcpy(hakswitch_last_played_path, line, sizeof(hakswitch_last_played_path));
   }
 ...
 * there's no free system slot left, see AUDIO_MIXER_MAX_SYSTEM_STREAMS
 * in audio_defines.h) so action_ok_hakswitch_open_file can fire it
 * with an instant, synchronous audio_driver_mixer_play_stream() call.
 * Loading it fresh from disk at the exact moment a game launches
 * (like a plain task_push_audio_mixer_load_and_play there would) loses
 * the race against content-load tearing down/reinitializing the audio
 * driver before the async decode finishes.
 *
 * Not a one-shot: "Close Content" fires CMD_EVENT_AUDIO_REINIT, which
 * tears down and rebuilds the whole audio driver and reloads
 * RetroArch's own OK/Cancel/scroll sounds (audio_driver_load_system_
 * sounds(), retroarch.c) - but has no idea our slot 0 exists, so it
 * comes back empty. Rather than hooking that event directly, this
 * just self-heals: called once a frame from ozone_frame() (see
 * hakswitch_check_pending_launch there) while sitting on the
 * carousel, it's a no-op unless the slot has actually gone empty. */
/* Same backoff reasoning as hakswitch_bgm_retry_cooldown below - a
 * file that never loads (state stuck at NONE) shouldn't get retried
 * every single frame forever. */
static int hakswitch_launch_sfx_retry_cooldown = 0;
#define HAKSWITCH_LAUNCH_SFX_RETRY_COOLDOWN_FRAMES 180

static void hakswitch_launch_sfx_load_cb(retro_task_t *task,
      void *task_data, void *user_data, const char *error)
{
   if (error)
      hakswitch_launch_sfx_retry_cooldown = HAKSWITCH_LAUNCH_SFX_RETRY_COOLDOWN_FRAMES;
}

void hakswitch_ensure_launch_sfx_loaded(void)
{
   char launch_sfx_path[1024];

   if (audio_driver_mixer_get_stream_state(HAKSWITCH_LAUNCH_SFX_SLOT)
         != AUDIO_STREAM_STATE_NONE)
      return;

   if (hakswitch_launch_sfx_retry_cooldown > 0)
   {
      hakswitch_launch_sfx_retry_cooldown--;
      return;
   }

   fill_pathname_join_special(launch_sfx_path, hakswitch_base_path,
         "sfx", sizeof(launch_sfx_path));
   fill_pathname_join_special(launch_sfx_path, launch_sfx_path,
         "launch.wav", sizeof(launch_sfx_path));

   if (filestream_exists(launch_sfx_path))
      task_push_audio_mixer_load(launch_sfx_path, hakswitch_launch_sfx_load_cb, NULL, false,
            AUDIO_MIXER_SLOT_SELECTION_MANUAL, HAKSWITCH_LAUNCH_SFX_SLOT);
   else
      hakswitch_launch_sfx_retry_cooldown = HAKSWITCH_LAUNCH_SFX_RETRY_COOLDOWN_FRAMES;
}

/* HAKSWITCH TEST: resolves every hakswitch path relative to wherever
 * retroarch itself lives, instead of one hardcoded absolute Windows
 * path - so the same source works on any PC the "hakswitch" folder is
 * copied next to the executable on, AND on Switch. On desktop that's
 * fill_pathname_application_dir() (reads argv[0]/GetModuleFileName);
 * that API is deliberately unavailable on consoles (its generic
 * implementation shells out to /proc, which doesn't exist there).
 * On Switch specifically, RetroArch's own platform driver
 * (frontend/drivers/platform_switch.c) hardcodes its OWN directory
 * (cores/info/config/etc.) to "/retroarch" regardless of where our
 * .nro or GLOBAL_CONFIG_DIR say - rather than fight that, our own
 * folders (consoles/icons/src) just live inside that same
 * "/retroarch" root too, so there's a single unified folder on the SD
 * card instead of two separate ones.
 * Runs once (guarded by hakswitch_base_path already being set) - safe
 * to call from anywhere that's about to use these paths. */
static void hakswitch_init_paths(void)
{
   if (*hakswitch_base_path)
      return;

#if defined(RARCH_CONSOLE)
   strlcpy(hakswitch_base_path, "/retroarch", sizeof(hakswitch_base_path));
#else
   fill_pathname_application_dir(hakswitch_base_path, sizeof(hakswitch_base_path));
   fill_pathname_join_special(hakswitch_base_path, hakswitch_base_path,
         "hakswitch", sizeof(hakswitch_base_path));
#endif

   fill_pathname_join_special(hakswitch_root_path, hakswitch_base_path,
         "consoles", sizeof(hakswitch_root_path));
...
/* Reserved user mixer slot for background music - distinct from
 * HAKSWITCH_LAUNCH_SFX_SLOT (0) above. -12dB is roughly a quarter
 * amplitude (20*log10(0.25) =~ -12dB) - quiet ambient bed under the
 * carousel, not full-volume like the sound effects. */
#define HAKSWITCH_BGM_SFX_SLOT   1
#define HAKSWITCH_BGM_VOLUME_DB -12.0f
/* Same value as audio_driver.c's own (private, not header-exposed)
 * MENU_SOUND_FORMATS - the set of extensions the audio mixer can
 * actually decode. */
#define HAKSWITCH_MUSIC_FORMATS "ogg|mod|xm|s3m|mp3|flac|wav"

/* Set while a load is in flight, so hakswitch_ensure_bgm_playing
 * (below) doesn't re-trigger another load every frame during the
 * (typically brief) window between kicking off the async decode and
 * the track actually reaching AUDIO_STREAM_STATE_PLAYING. */
static bool hakswitch_bgm_load_pending = false;
/* Remembers the last track picked, so a fresh pick (e.g. right after
 * "Close Content" stops the previous one) never repeats it back to
 * back - with only a couple of files in the folder, plain random_range
 * would otherwise re-roll the same song roughly half the time and
 * just look broken. */
static char hakswitch_bgm_last_path[1024] = "";
/* Frames left to wait before hakswitch_ensure_bgm_playing will try
 * loading another track, set after a failed load. Without this, a
 * track that fails every time (bad file, unsupported codec, ...)
 * would retry on every single frame forever - repeatedly spinning up
 * the task system with no gap is exactly the kind of tight loop that
 * can exhaust real hardware, so it backs off instead. */
static int hakswitch_bgm_retry_cooldown = 0;
#define HAKSWITCH_BGM_RETRY_COOLDOWN_FRAMES 180

static void hakswitch_bgm_load_cb(retro_task_t *task,
      void *task_data, void *user_data, const char *error)
{
   hakswitch_bgm_load_pending = false;

   if (error)
   {
      hakswitch_bgm_retry_cooldown = HAKSWITCH_BGM_RETRY_COOLDOWN_FRAMES;
      return;
   }

   /* task_push_audio_mixer_load(_and_play) always starts new voices
    * at unity gain internally - the only way to actually apply a
    * reduced volume is audio_driver_mixer_set_stream_volume() on an
    * already-playing stream, once its voice exists. Play first, then
    * turn it down. */
   audio_driver_mixer_play_stream(HAKSWITCH_BGM_SFX_SLOT);
   audio_driver_mixer_set_stream_volume(HAKSWITCH_BGM_SFX_SLOT,
         HAKSWITCH_BGM_VOLUME_DB);
}

/* Keeps something playing from hakswitch_base_path/music/ at all
 * times - self-healing exactly like hakswitch_ensure_launch_sfx_loaded
 * above: called once a frame from ozone_frame() while the carousel is
 * up, it's a no-op unless the BGM slot isn't currently playing
 * (previous track ended naturally, hakswitch_stop_bgm below just ran,
 * or "Close Content"'s CMD_EVENT_AUDIO_REINIT wiped it), in which case
 * it (re-)scans the music folder and picks a fresh random track.
 * Dropping more files into that folder later needs no code changes -
 * it's scanned fresh every time a track is needed, never cached.
 *
 * Known limitation: a large (~19MB) mp3 was found to hang the async
 * load indefinitely (audio_driver_mixer_add_stream never gets called
 * with a final state one way or the other) - suspected bug in the
 * windowed-load path for big files, not chased down further. Keep
 * ambient tracks reasonably short/compressed until that's root-caused. */
void hakswitch_ensure_bgm_playing(void)
{
   char music_dir[1024];
   struct string_list *tracks;
   enum audio_mixer_state state;

   if (hakswitch_bgm_load_pending)
      return;

   if (hakswitch_bgm_retry_cooldown > 0)
   {
      hakswitch_bgm_retry_cooldown--;
      return;
   }

   state = audio_driver_mixer_get_stream_state(HAKSWITCH_BGM_SFX_SLOT);
   if (     state == AUDIO_STREAM_STATE_PLAYING
         || state == AUDIO_STREAM_STATE_PLAYING_LOOPED
         || state == AUDIO_STREAM_STATE_PLAYING_SEQUENTIAL)
      return;

   fill_pathname_join_special(music_dir, hakswitch_base_path,
         "music", sizeof(music_dir));

   tracks = dir_list_new(music_dir, HAKSWITCH_MUSIC_FORMATS, false, false, false, false);
   if (tracks)
   {
      if (tracks->size > 0)
      {
         /* Deliberately not random_range()/rand(): rand() is reseeded
          * with srand(time(NULL)) once per process (retroarch.c), and on
          * Switch "Close Content" re-execs back into this launcher as a
          * brand new process (see hakswitch_check_pending_launch in
          * menu_cbs_ok.c) - time() only has 1-second resolution, so a
          * return trip that completes inside the same wall-clock second
          * reseeds to the exact same value and picks the exact same
          * "random" track every time. cpu_features_get_time_usec() reads
          * a free-running hardware tick that never resets across process
          * launches and already has proven microsecond-level resolution
          * (used for the launch-sfx delay), so it doesn't have that
          * failure mode. */
         size_t pick = (size_t)(cpu_features_get_time_usec() % (retro_time_t)tracks->size);

         /* Re-roll on a match against the last track, so returning to
          * the carousel doesn't just so happen to replay the same
          * song - always terminates, since with size > 1 there's
          * always at least one other index that isn't the match. */
         if (tracks->size > 1)
            while (string_is_equal(tracks->elems[pick].data, hakswitch_bgm_last_path))
               pick = (pick + 1) % tracks->size;

         strlcpy(hakswitch_bgm_last_path, tracks->elems[pick].data,
               sizeof(hakswitch_bgm_last_path));

         hakswitch_bgm_load_pending = true;
         task_push_audio_mixer_load(tracks->elems[pick].data,
               hakswitch_bgm_load_cb, NULL, false,
               AUDIO_MIXER_SLOT_SELECTION_MANUAL, HAKSWITCH_BGM_SFX_SLOT);
      }
      dir_list_free(tracks);
   }
}

/* Ambient music only belongs on the carousel - called from
 * ozone_frame() whenever hakswitch_on_main_menu is false (Quick Menu,
 * any other tab, or gameplay). No real "pause" in this mixer API;
 * stopping is fine here since hakswitch_ensure_bgm_playing above
 * already treats "not playing" as "go pick a fresh track", so coming
 * back to the carousel just starts a different song next. */
void hakswitch_stop_bgm(void)
{
   audio_driver_mixer_stop_stream(HAKSWITCH_BGM_SFX_SLOT);
}
      ...
      bool include_everything)
{
   ...
   switch (type)
   {
      ...
      case DISPLAYLIST_CONTENT_SETTINGS:
         /* HAKSWITCH: replaces RetroArch's own long Quick Menu (Resume,
          * Restart, Close Content, Save/Load State, Options, Controls,
          * Screenshot, Recording, Favorites, Shaders, Cheats, Netplay,
          * Achievements, Overrides, ...) with a fixed 5-item list. The
          * full stock experience stays one tap away via "Settings"
          * below, which pushes MENU_ENUM_LABEL_SETTINGS - the same
          ...
          * slot, that's what the full Settings screen is for. */
         {
            ...
            if (menu_entries_append(list,
                     msg_hash_to_str(MENU_ENUM_LABEL_VALUE_SETTINGS_TAB),
   ...
   if (!push_list)
   {
      ...
      switch (type)
      {
         ...
         case DISPLAYLIST_MAIN_MENU:
            menu_entries_clear(info->list);
            hakswitch_init_paths();
            /* HAKSWITCH TEST: browse hakswitch_current_path (see its
             * declaration above) instead of the normal Load Core/
             * Settings/etc. entries below. Folders get the "hakswitch_dir"
             * label and files get "hakswitch_file" so menu_cbs_ok.c can
             * recognize and intercept OK presses on them (see
             * action_ok_hakswitch_enter_dir / _go_up / _open_file). A
             * Going back up is a separate on-screen button (drawn below
             * the carousel in ozone.c, reachable with Down), not a list
             * entry - the user specifically asked for it to live below
             * the carousel rather than mixed in with the games. */
            {
               struct string_list *hakswitch_dirs = dir_list_new(
                     hakswitch_current_path, NULL, true, false, false, false);
               /* HAKSWITCH TEST: if a game was just launched from this
                * exact list, find it again by full path (not just index -
                * the list is freshly rescanned, so basing this on an
                * index would break if scan order ever changed) and
                * re-select it, instead of leaving it at whatever
                * RetroArch's own "return to Main Menu" flow reset the
                * selection to. */
               int hakswitch_restore_idx = -1;

               if (hakswitch_dirs)
               {
                  unsigned hi;
                  dir_list_sort(hakswitch_dirs, true);
                  for (hi = 0; hi < hakswitch_dirs->size; hi++)
                  {
                     /* dir_list marks directories with RARCH_DIRECTORY (a
                      * different enum than the menu's FILE_TYPE_DIRECTORY,
                      * despite the similar name) - only the entry's TYPE
                      * argument below uses FILE_TYPE_DIRECTORY. */
                     bool is_dir  = hakswitch_dirs->elems[hi].attr.i == RARCH_DIRECTORY;
                     menu_entries_append(info->list,
                           path_basename(hakswitch_dirs->elems[hi].data),
                           is_dir ? "hakswitch_dir" : "hakswitch_file",
                           MSG_UNKNOWN, FILE_TYPE_DIRECTORY, 0, 0, NULL);

                     if (hakswitch_last_played_path[0] && string_is_equal(
                              hakswitch_dirs->elems[hi].data, hakswitch_last_played_path))
                        hakswitch_restore_idx = (int)hi;
                  }
                  dir_list_free(hakswitch_dirs);
               }

               if (hakswitch_restore_idx >= 0)
               {
                  struct menu_state *hakswitch_menu_st = menu_state_get_ptr();
                  hakswitch_menu_st->selection_ptr = (size_t)hakswitch_restore_idx;
                  /* Deliberately NOT cleared here. RetroArch builds the
                   * Main Menu displaylist at least once during its own
                   * generic startup - before auto-starting content -
                   * even when that screen is never actually shown to
                   * the user (process boots straight into the game).
                   * Clearing on first match would consume the restore
                   * on that invisible pass, leaving nothing left to
                   * apply on the rebuild that happens after the user
                   * really does "Close Content" and sees this screen.
                   * Cleared instead the moment the user actually
                   * navigates elsewhere (see action_ok_hakswitch_enter_dir
                   * in menu_cbs_ok.c and the "go back up" handler in
                   * ozone.c), so it keeps re-applying safely across any
                   * number of incidental rebuilds until then. */
               }
